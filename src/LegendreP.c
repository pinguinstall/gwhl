...
// computeAssociatedLegendre:=
//      proc(x,lmax)
//        local sx,P,m,l:
//        sx:=sqrt(1-x^2):
//        P[0,0]:=1:
//        for m from 0 to lmax-1 do
//          P[m+1,m+1]:=evalf((2*m+1)*sx*P[m,m]):
//        #end do:
//        #for m from 0 to lmax-1 do
//          P[m+1,m]:=evalf((2*m+1)*x*P[m,m]):
//        end do:
//        for m from 0 to lmax-2 do
//          for l from m+2 to lmax do
//            P[l,m]:=evalf(((2*l-1)*x*P[l-1,m]-(l-1+m)*P[l-2,m])/(l-m)):
//          end do: 
//        end do:
//        RETURN(P):
//      end proc:
...
//     B[1,1]:=1:
//     for m from 1 to lmax-1 do
//                B[m+1,m+1]:=evalf((2*m+1)*(m+1)/m*sx*B[m,m]):
//                B[m+1,m]:=evalf((2*m+1)*x*B[m,m]):
//     end do:
...
//     for m from 1 to lmax-2 do
//       for l from m+2 to lmax do
// 	B[l,m]:=evalf(((2*l-1)*x*B[l-1,m]-(l-1+m)*B[l-2,m])/(l-m)):
//       end do: 
//     end do:
    for(m = 1; m <= lmax-2; m++){
	for(l = m + 2; l <= lmax; l++){
...
// 	if l=m then 
// 	  A[l,m]:=evalf(-l*x*B[l,m]/m):
// 	else
// 	  A[l,m]:=evalf((-l*x*B[l,m]+(l+m)*B[l-1,m])/m):                   
// 	end if:
//       end do: 
//     end do:    
    for(l = 1; l <= lmax; l++){
